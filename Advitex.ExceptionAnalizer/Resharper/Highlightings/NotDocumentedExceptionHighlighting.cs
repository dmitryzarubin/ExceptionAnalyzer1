...
namespace Advitex.ExceptionAnalizer.Resharper.Highlightings
{
    ...
    public class NotDocumentedExceptionHighlighting : CSharpHighlightingBase, IHighlighting
    {
        public NotDocumentedExceptionHighlighting([NotNull] ThrownException exception)
        {
            if (exception == null)
            ...
            ToolTip = string.Format("Exception of type {0} is not documented [Exception Analyzer]",
                                    Exception.ShortTypeName);
        }

        public const string SeverityId = "NotDocumentedExceptionHighlighting";

...
namespace Advitex.ExceptionAnalizer.Resharper.Highlightings
{
    ...
    public class PossibleNotDocumentedThrowHighlighting : CSharpHighlightingBase, IHighlighting
    {
        public PossibleNotDocumentedThrowHighlighting([NotNull] ThrownException exception)
        {
            if (exception == null)
                throw new ArgumentNullException("exception");

            Exception = exception;
            ToolTip = string.Format("Perhaps, thrown exception of type {0} is not documented [Exception Analyzer]",
                                    Exception.ShortTypeName);
        }

        public const string SeverityId = "PossibleNotDocumentedThrowHighlighting";
